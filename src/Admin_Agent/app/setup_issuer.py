...
try:
    ...
    resp = requests.post(URL+"/connections/create-invitation", headers=header, timeout=20)
    body = resp.json()
...
try:
    ...
    resp_accept = requests.post(URL_holder+"/connections/receive-invitation", data=conn_invite, headers=header, timeout=20)
...
try:
    resp_confirm_active = requests.get(URL+"/connections/"+connection_id, timeout=20)
