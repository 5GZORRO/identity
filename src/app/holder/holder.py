...
async def register_stakeholder(response: Response, body: Stakeholder):
    # PRIVATE DID
    try:
...
async def update_stakeholder_state(request_id: str, body: dict, response: Response):
    #UPDATE MONGO RECORD
    try:
        mongo_setup_provider.stakeholder_col.find_one_and_update({'_id': ObjectId(request_id)}, {'$set': {"state": "Stakeholder Registered"}}) # UPDATE REQUEST RECORD FROM MONGO
